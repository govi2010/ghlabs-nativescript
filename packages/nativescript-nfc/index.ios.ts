...
class NFCNDEFReaderSessionDelegateImpl extends NSObject implements NFCNDEFReaderSessionDelegate {
  ...
  private static recordToJSON(record: NFCNDEFPayload): NfcNdefRecord {
    ...
    return {
      ...
      payload: JSON.stringify(NFCNDEFReaderSessionDelegateImpl.hexToDecArray(payloadAsHexArray)),
  ...
  private static hexToDec(hex: string): number | undefined {
    if (hex === undefined) {
      return undefined;
    }
  ...
  private static buf2hexString(buffer: ArrayBuffer): string {
    // buffer is an ArrayBuffer
    return Array.prototype.map.call(new Uint8Array(buffer), (x: number) => ('00' + x.toString(16)).slice(-2)).join('');
  }

  private static buf2hexArray(buffer: ArrayBuffer): Array<string> {
    // buffer is an ArrayBuffer
    return Array.prototype.map.call(new Uint8Array(buffer), (x: number) => ('00' + x.toString(16)).slice(-2));
  }

  private static buf2hexArrayNr(buffer: ArrayBuffer): Array<number> {
    // buffer is an ArrayBuffer
    return Array.prototype.map.call(new Uint8Array(buffer), (x: number) => +x.toString(16));
  }

  private static hex2a(hexx: any): string {
